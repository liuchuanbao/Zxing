 ...
 *
 * < p >这类尝试找到一个二维码仪模式。仪模式是广场
 *标记三个二维码的角落。< / p >
 *
 * < p >这个类是线程安全的,但不可重入的。每个线程必须分配自己的对象。
 *
 * @author肖恩·欧文
 *
 * @author Sean Owen
...
/**
 *v我们正在寻找黑色/白色/黑色/白色/黑色模块
 / / 1:1:3:1:1比率;这追踪这种模块的数量
 / /假设最大版本二维码我们支持占1/4的高度
 / /图像,然后占3模块大小的中心。这给最小的
 / /数量的像素中心,所以经常跳过这。当努力,寻找所有
 / /不管他们有多密集QR版本。
 */
    ...
    for (int i = iSkip - 1; i < maxI && !done; i += iSkip) {
      ...
      for (int j = 0; j < maxJ; j++) {
        ...
        } else { // White pixel 白色像素
          if ((currentState & 1) == 0) { // Counting black pixels  //计数黑色
            if (currentState == 4) { // A winner?
              if (foundPatternCross(stateCount)) { // Yes
                boolean confirmed = handlePossibleCenter(stateCount, i, j, pureBarcode);
                if (confirmed) {
                  // Start examining every other line. Checking each line turned out to be too
                  // expensive and didn't improve performance./ /开始检查每隔一行。检查每一行了/ /昂贵和不提高性能。
 ...
 * /跳过行之间行较低的中心确认
 ...
 / /但备份有点全面检测的机会
 / /它,整个宽度仪中心模式
 / / rowSkip跳过,但是回到了stateCount[2](大小的中心
 / /模式我们看到)的保守,也iSkip后退的
 / /将要重新添加
 */
                ...
                // Clear state to start looking again 明确的状态重新开始寻找
                currentState = 0;
                stateCount[0] = 0;
                stateCount[1] = 0;
                stateCount[2] = 0;
          ...
          } else { // Counting white pixels //计数白色像素
            stateCount[currentState]++;
          }
        }
      }
