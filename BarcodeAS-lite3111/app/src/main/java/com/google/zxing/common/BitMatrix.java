 ...
 * efficiently.</p>
 *< p >是一个二维矩阵的碎片。下面的函数参数,整个常见
 *模块,x是列位置,y是一行的位置。订购总是x,y。
 *原点在左上角。< / p >
 *
 * < p >内部位表示一个32位整数的一维数组。然而,每一行的开始
 *新int。这样做是故意这样我们可以复制出一个行BitArray非常
 *有效。< / p >
 *
 * < p >行位的排序。在每一个int,首先,使用最低有效位
 *意味着他们代表x值较低。这是兼容BitArray实现。< / p >
 *
 *
 *
...
public final class BitMatrix implements Cloneable {
  ...
  public static BitMatrix parse(String stringRepresentation, String setString, String unsetString) {
    ...
    Log.e("xyc"," BitMatrux = parse+ "+matrix.toString());
    return matrix;
  }
...
//    Log.e("xyc","BitMatrix  set "+offset);
    bits[offset] |= 1 << (x & 0x1f);
//    Log.e("xyc","BitMatrix  set "+bits[offset]);
  }
  ...
  public BitArray getRow(int y, BitArray row) {
    ...
    Log.e("xyc","BitMatrix getRow "+row.toString());
    return row;
  }
   ...
   * 修改这个{ @code BitMatrix }表示相同的但旋转了180度
   */
   ...
   **这是有用的在检测“纯粹”的封闭矩形条形码。
   *
   * @return { @code左,上,宽度、高度}封闭矩形的1位,或null如果它是白色的
   ...
   *
    etString表示一组
   *unsetString表示的设置
   * lineSeparator换行符的字符串表示
   * 整个矩阵利用给定的字符串表示字符串和行分隔符
   叫{ @link # toString(字符串,字符串)},它使用\ n行分隔符
   *
//   * @param lineSeparator newline character in string representation
  ...
  @Deprecated
  public String toString(String setString, String unsetString,int ear) {
    StringBuilder result = new StringBuilder(height* (width + 1));
