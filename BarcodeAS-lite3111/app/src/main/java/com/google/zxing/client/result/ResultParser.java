 ...
 *抽象类代表条形码解码的结果,超过
 *一个字符串,作为某种类型的结构化数据。这可能代表一个子类
 *一个URL,或者一个电子邮件地址。{ @link # parseResult(结果)}将原始
 *解码字符串到最合适的类型的结构化表示。< / p >
 *
 * < p >感谢杰夫Griffin提议改写这些类的,更少的依赖
 *在异常机制解析。< / p >
...
public abstract class ResultParser {
   ...
   *
   * 尝试解析原始} { @link结果作为一个特定类型的内容
   *的信息(电子邮件、网址等)并返回一个{ @link ParsedResult }封装
   *解析的结果。
   *
...
//对矩阵进行解码
  public static ParsedResult parseResult(Result theResult) {
    ...
    for (ResultParser parser : PARSERS) {
      ...
      if (result != null) {
        Log.e("xyc","ResultParser = +"+result.toString());
        return result;
      }
    }
