...
final class BitMatrixParser {
   ...
   * QR Code.</p>
   *在{ < p >读取位@link BitMatrix }代表仪模式中
   *正确的顺序,以便重建中包含的字节码字
   *二维码。< / p >
   *
   在二维码* @return字节编码
   *  FormatException如果准确的预期不读的字节数
   * @bytes encoded within the QR Code
   * @t FormatException if the exact number of bytes expected is not read
   */
